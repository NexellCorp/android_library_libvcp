...
/************************************************************************/
typedef signed short    pitem_t;            /* 16 bits */
...
typedef struct err_s {
    signed short err;                                    /* error's id */
    signed short pid;                                    /* error profile's ID */
    signed short memb;                                   /* member number */
} err_t;
...
ARGS:
mem_reg_t * reg    --  pointer to memory region structure(s).
...
/************************************************************************
FUNCTION:
vcp_get_data()     --  Gets object's value in VCP run-time mode
ARGS:
mem_reg_t * reg    --  pointer to memory region structure(s).
pcm_t param        --  the value. see definitions below
pcm_t *value       --  the object's value that will be read
ATTN! pcm_t type is used!
RETURNS:
 0 : ok
-1 : Wrong ID
-2 : VCP8 object isn't initialized
-3 : Block is turned off (e.g. ADM block is turned off)
-4 : This processing is turend off (e.g. ADM block is on but WNR processing is off)
* ***********************************************************************/

extern int vcp_get_data(void *reg, pitem_t id, pitem_t *value);

/************************************************************************
FUNCTION:
read_binary_prof_rt()  --  converts binary profile to actual profile
ARGS:
void * pmem            --  pointer to source array containing binary profile
RETURNS:
 pointer to actual profile
* ***********************************************************************/

vcp_profile_t *read_binary_prof_rt(void *pmem);

/************************************************************************
FUNCTION:
create_binary_profile_rt()  --  converts actual profile to binary profile
ARGS:
vcp_profile_t *prof         --  pointer to actual profile
void * pmem                 --  pointer to destination array 
                                to store binary profile
RETURNS:
  size of binary profile in bytes
* ***********************************************************************/

unsigned int create_binary_profile_rt(vcp_profile_t *prof, void *mem);

/************************************************************************
FUNCTION:
compute_crc16()             --  compute CRC16
ARGS:
vcp_profile_t *p            --  pointer to actual profile
* ***********************************************************************/

void compute_crc16(vcp_profile_t *p);

/* below are listed object's data (pitem_t param) for functions vcp_get_data() */
#define VCP_DATA_ADM_WIND_INDEX        1

#define VCP_GETDATA_OK             0
#define VCP_GETDATA_WRONG_ID      -1    /* Wrong ID */
#define VCP_GETDATA_NO_INIT       -2    /* VCP8 object isn't initialized */
#define VCP_GETDATA_BLOCK_OFF     -3    /* Block is turned off (e.g. ADM block is turned off) */
#define VCP_GETDATA_PROCESS_OFF   -4    /* This processing is turend off (e.g. ADM block is on but WNR processing is off) */
